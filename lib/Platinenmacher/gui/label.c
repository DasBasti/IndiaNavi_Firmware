        ...
        if (label->roundedCorners == 0) {
            if (label->borderLines & TOP_SOLID)
                display_line_draw(dsp, label->box.left, label->box.top,
                    label->box.left + width, label->box.top,
                    label->borderColor);

            if (label->borderLines & BOTTOM_SOLID)
                display_line_draw(dsp, label->box.left, label->box.top + height,
                    label->box.left + width,
                    label->box.top + height, label->borderColor);
            ...
            if (label->borderLines & RIGHT_SOLID)
                display_line_draw(dsp, label->box.left + width, label->box.top,
                    label->box.left + width,
                    label->box.top + height, label->borderColor);
        } else { // Set all 4 Lines onto the 4 corners
            ...
            point_t t2 = { label->box.left + width, label->box.top };
            point_t r1 = { label->box.left + width, label->box.top };

            point_t l2 = { label->box.left, label->box.top + height };
            point_t b1 = { label->box.left, label->box.top + height };

            point_t b2 = { label->box.left + width, label->box.top + height };
            point_t r2 = { label->box.left + width, label->box.top + height };

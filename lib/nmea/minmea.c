...
#define next_field()                            \
	...
	while (*format)
	{
		...
		{ // Fractional value with scale (struct minmea_float).
			...
			*va_arg(ap, struct minmea_float *) = (struct minmea_float){value,
																	   scale};
		}
		break;
...
bool minmea_parse_rmc(struct minmea_sentence_rmc *frame, const char *sentence)
{
	...
	if (!minmea_scan(sentence, "tTcfdfdffDfd", type, &frame->time, &validity,
					 ...
					 &longitude_direction, &frame->speed, &frame->course, &frame->date,
...
bool minmea_parse_txt(struct minmea_sentence_txt *frame, const char *sentence)
{
	...
	if (!minmea_scan(sentence, "tiii;s", type, &frame->total, &frame->number,
					 &frame->identifier, &frame->text))
		return false;
	if (strcmp(type + 2, "TXT"))
		return false;
	// check chars
